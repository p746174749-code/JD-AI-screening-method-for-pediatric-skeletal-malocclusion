...
# Importing custom multi-input ConvNeXt variants
...
class EarlyStopping:
    """
    Stops training if validation accuracy doesn't improve after a set number of epochs.
    ...
    def __init__(self, patience=15, delta=0):
        ...
        self.counter = 0  # Tracks epochs since last improvement
        self.best_score = None  # Stores the highest validation accuracy found
        self.early_stop = False  # Flag to signal training termination
    ...
    def __call__(self, val_acc):
        ...
        if self.best_score is None:
            ...
            # If accuracy did not increase significantly
            self.counter += 1
            print(f'EarlyStopping counter: {self.counter} out of {self.patience}')
            if self.counter >= self.patience:
                self.early_stop = True
        else:
            # New best accuracy found; reset counter
            self.best_score = score
            self.counter = 0


# ================= 2. Evaluation & Logging Helpers =================
def setup_logger(output_dir, args):
    """Initializes logging to record training configuration and epoch results."""
...
def compute_metrics(all_labels, all_preds, num_classes):
    """Calculates per-class Accuracy, Sensitivity (Recall), and Specificity."""
    ...
    for i in range(num_classes):
        ...
        # Avoid division by zero with conditional checks
...
def plot_confusion_matrix(cm, epoch, run_dir, writer, num_classes):
    """Generates a visual Confusion Matrix and saves it to disk and TensorBoard."""
...
def train(model, args, device):
    ...
    # Dataset and Loader Initialization
    ...
    # LR Scheduler Logic: Linear Warmup followed by Cosine Annealing decay
    def lr_lambda(ep):
        if ep < args.warmup_epochs:
            # Linear increase from 0 to initial LR
            return float(ep) / float(max(1, args.warmup_epochs))
        else:
            # Cosine decay towards the minimum LR ratio
    ...
    # Directory Management for results
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, labels in tqdm(train_loader, desc=f"{args.fold} Epoch {epoch}/{args.epochs}"):
            # Transfer multi-input images to GPU
            images = [img.to(device) for img in images]
            labels = labels.to(device)

            optimizer.zero_grad()
            logits = model(*images)  # Forward pass: unpacking the images tuple
        ...
        # Validation phase: Evaluating model performance
        model.eval()
        correct, total = 0, 0
        ...
        # Telemetry: Update TensorBoard scalars
        ...
        # Save weights: Best and Last versions
...
def parse_args():
    parser = argparse.ArgumentParser(description="Multi-input ConvNeXt Training Script")
    parser.add_argument('--dataset_dir', type=str, required=True, help="Path to the dataset")
    parser.add_argument('--num_classes', type=int, required=True, help="Number of classification categories")
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--learning_rate', type=float, default=0.0001)
    parser.add_argument('--gpu', type=int, default=0, help="Target GPU index")
    parser.add_argument('--image_indices', type=int, nargs='+', required=True,
                        help="Indices of images to load (e.g. 2 3 4)")

    # Metadata for path organization
    parser.add_argument('--run_name', type=str, default="Default_Exp")
...
if __name__ == '__main__':
    ...
    # Select model based on number of inputs defined in image_indices
    num_inputs = len(args.image_indices)
    model_factory = {2: ConvNeXt_T2, 3: ConvNeXt_T3, 4: ConvNeXt_T4}

    if num_inputs not in model_factory:
        raise ValueError(f"No model architecture supports {num_inputs} inputs!")

    # Instantiate model and send to device
    model = model_factory[num_inputs](num_classes=args.num_classes).to(device)
